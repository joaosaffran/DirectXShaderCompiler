...
failing test names) are recorded per-suite in rc-manifest.json, alongside the DXC
commit, the SPIRV submodule commits, and the lavapipe / WARP driver versions. With
--allow-test-failures the script still exits 0 so the pipeline publishes the release
candidate regardless; whether a candidate is good enough to release is the team's
call, read from the recorded results.
...
def write_manifest(build_dir, dxc_sha, suites, lavapipe_version, warp_version):
    # No top-level pass/fail verdict: whether a candidate is good enough to release
    # is the team's decision, read from the per-suite results below -- not a flag the
    # tooling sets. lavapipe_version / warp_version are the software drivers the offload
    # tests run the candidate against: lavapipe (Mesa) for SPIRV, WARP for DXIL.
    manifest = {
        "dxc_commit": dxc_sha,
...
def main(argv=None):
    ...
    parser.add_argument("--lavapipe-version", default="",
                        help="lavapipe (Mesa) version the Vulkan offload tests run "
                             "against (recorded in the manifest).")
    parser.add_argument("--warp-version", default="",
                        help="WARP version the Direct3D 12 offload tests run against "
    ...
    manifest = write_manifest(build_dir, git_head(), suites,
                              args.lavapipe_version, args.warp_version)
    ...
    if any_problem:
        print("\nPublishing release candidate anyway (--allow-test-failures); "
              "the per-suite failures are recorded in the manifest for the team to review.")
    return 0
